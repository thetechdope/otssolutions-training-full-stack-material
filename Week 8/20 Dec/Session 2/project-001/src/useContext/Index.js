	...
	return (
		<div className="ui container">
			<div>
				...
				<br />
				<br />
				<Input placeholder="Enter Amount..." onChange={(event) => setAmount(event.target.value)} />
				<br />
				<br />
				Payment Mode :
				<select
					placeholder="Select Payment"
					style={{ width: '20%', padding: 10, fontSize: 15, fontWeight: 'bold' }}
					onChange={(e) => setPaymentMode(e.target.value)}
				>
					<option defaultValue disabled>
						Select Payment
					</option>
					{['Cash', 'UPI', 'Debit Card/Credit Card'].map((item) => (
						<option style={{ width: '20%', padding: 10, fontSize: 15 }} value={item}>
							{item}
						</option>
					))}
				</select>
